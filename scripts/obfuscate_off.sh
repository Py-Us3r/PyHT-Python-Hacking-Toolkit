...
wine cmd /c "pyinstaller --onefile --noconsole --hidden-import=requests -i scripts/transparente.ico scripts/${SCRIPT_NAME}"
mv dist/SecurityCheck.exe .
bash scripts/cert.sh SecurityCheck.exe
rm -rf dist/ build/ SecurityCheck.spec $PYTHON_INSTALLER scripts/SecurityCheck.py
